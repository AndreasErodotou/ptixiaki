...
class ListingsApi extends Component {
    ...
    listingClickedHandler(LID){
        const fullListing=this.state.listings.filter((listing) => listing.LID === LID);
    ...
    render() {
        const listing=this.state.listings.filter((listing) => listing.LID === this.state.listingClickedId);
