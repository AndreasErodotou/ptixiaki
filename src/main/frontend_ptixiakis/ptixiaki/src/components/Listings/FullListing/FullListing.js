...
const FullListing = ({listing,showFullListing,...handleModal}) => {
    ...
    if(listing[0].pics !== null && listing[0].pics !== undefined){
        otherPics = listing[0].pics.map((pic, indexed) =>{
            if(indexed===0){
                return <img className="img-responsive pic col-sm-12" id="mainPic"  src={(pic)?pic:data}  alt="pic"></img>
            }
            return <img key= {indexed}className="img-responsive pic col-sm-3"  src={(pic)?pic:data}  alt={"pic"+indexed}></img>
        });
    }
                    ...
                    <div className="container" id="simpleFullListing">
                        <div className="border border-info row" id="fullListing">
                            ...
                            <div className="col-sm-8" id="info">
                                ...
                                <h6 ><small>{listing[0].description}</small></h6>
                                ...
                                <h6 id="posted_from"> posted from: {listing[0].available_from} </h6>
                                <h6 id="available"> available until: {listing[0].available_until} </h6>
